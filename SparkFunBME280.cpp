...
- Configuration setup is done in begin()
to apply I2C interface and the highest oversampling rate 
...
BME280::BME280( void )
{
	...
	settings.I2CAddress = 0x76; // can be also 0x77    Ignored for SPI_MODE
...
Wire.begin() must be done in user code
************************************************/
...
// 4, coefficients = 16
*****************************************************/
	settings.filter = 4; //Lots of HW filter  x16 is good for pressure average (see ds page 19)

/***********humidOverSample can be: *********************
// 0, skipped
// 1 through 5, oversampling *1, *2, *4, *8, *16 respectively
******************************************************/
	settings.humidOverSample = 0;	// why '1', better '0' as we don;t need humidity reading

/***************tempOverSample can be: ******************
// 0, skipped
// 1 through 5, oversampling *1, *2, *4, *8, *16 respectively
******************************************************/
	settings.tempOverSample = 1;		// why '5', better '1' for x1 oversampling (see ds page 19)	

/************pressOverSample can be: *********************
// 0, skipped
// 1 through 5, oversampling *1, *2, *4, *8, *16 respectively
*******************************************************/
	settings.pressOverSample = 4;	// why "5", better '4' for x8 oversampling (see ds page 19)
...
float BME280::readFloatPressure( void )
{
	...
	readTempC();	// to get t_fine for pressure calc
...
// provide sea level pressure in float Pa
// returns Altitude in float meters
float BME280::readFloatAltitudeMeters(float pressure = 101300.00)
{
	float heightOutput = 0;

//	heightOutput = ((float)-45846.2)*(pow(((float)readFloatPressure()/pressure), 0.19026) - (float)1);
	heightOutput = ((float)-44330.0)*(pow(((float)readFloatPressure() / pressure), 0.19026) - (float)1);
	return heightOutput;
...
float BME280::readFloatAltitudeFeet(  float pressure )
{
	float heightOutput = 0;
	
	heightOutput = readFloatAltitudeMeters(pressure) * 3.28084;
	return heightOutput;
